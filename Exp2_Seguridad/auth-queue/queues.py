"""
auth-queue: Redis Stream consumer.

This script connects to a Redis Stream, listens for new messages, and forwards them to a monitoring service
via HTTP POST requests. It implements retry logic with exponential backoff for failed requests and
acknowledges messages in the stream only after successful forwarding.
"""
...
MONITOR_URL = os.getenv("MONITOR_URL", "http://auth-anomaly:6005/auth-event")
STREAM_NAME = os.getenv("STREAM_NAME", "reports")
CONSUMER_GROUP = os.getenv("CONSUMER_GROUP", "auth-anomaly-queue-group")
...
def _ensure_group(r: redis.Redis) -> None:
    try:
        ...
        log.info(
            "Created consumer group '%s' on stream '%s'", CONSUMER_GROUP, STREAM_NAME
        )
...
def run() -> None:
    ...
    _ensure_group(r)
    log.info(
        "Listening on stream '%s' as '%s/%s' → %s",
        STREAM_NAME,
        CONSUMER_GROUP,
        CONSUMER_NAME,
        MONITOR_URL,
        ...
        for _stream, messages in results:
            for msg_id, fields in messages:
                ...
                try:
                    ...
                    log.info(
                        "ACK %s  service=%s  status=%s",
                        msg_id,
                        payload_obj.get("service"),
                        payload_obj.get("status"),
                    )
